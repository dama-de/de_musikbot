...
from discord.ext.commands import Bot, Cog, CommandError, CommandInvokeError, Context, MissingRequiredArgument, command, \
    group, hybrid_command, hybrid_group
import pylast
import tekore
...
class Music(Cog):
    ...
    @last.command()
    async def tracks(self, ctx: Context, period="all"):
    ...
    @last.command()
    async def albums(self, ctx: Context, period="all"):
    ...
    @last.command()
    async def artists(self, ctx: Context, period="all"):
    ...
    # ---------- Slash Commands ----------
    # @command(name="album", description="Search for an album")
    # @describe(search_query="name of the album")
    # async def _album(self, ctx: Context, search_query=""):
    #     await ctx.defer()
    #     await self.album(ctx, search_query=search_query)
    #
    # @command(name="artist", description="Search for an artist")
    # @describe(search_query="name of the artist")
    # async def _artist(self, ctx: Context, search_query=""):
    #     await ctx.defer()
    #     await self.artist(ctx, search_query=search_query)
    #
    # @command(name="track", description="Search for a track")
    # @describe(search_query="name of the track")
    # async def _track(self, ctx: Context, search_query=""):
    #     await ctx.defer()
    #     await self.track(ctx, search_query=search_query)
    #
    # @group(name="last")
    # async def _last(self, ctx: Context):
    #     pass
    #
    # @_last.command(name="register", description="Register your last.fm account with the bot")
    # @describe(lastfm_name="Your last.fm username")
    # async def _register(self, ctx: Context, lastfm_name):
    #     await self.register(ctx, lastfm_name)
    #
    # @_last.command(name="now", description="Fetch the currently playing song")
    # async def _now(self, ctx: Context):
    #     await ctx.defer()
    #     await self.now(ctx)
    #
    # @_last.command(name="recent", description="Fetch your last 10 scrobbles")
    # async def _recent(self, ctx: Context):
    #     await ctx.defer()
    #     await self.recent(ctx)
    #
    # @_last.command(name="artists", description="Fetch your most played artists")
    # @describe(period="Time period")
    # async def _artists(self, ctx: Context, period: Literal["all", "7d", "1m", "3m", "6m", "12m"] = "all"):
    #     await ctx.defer()
    #     await self.artists(ctx, period)
    #
    # @_last.command(name="albums", description="Fetch your most played albums")
    # @describe(period="Time period")
    # async def _albums(self, ctx: Context, period: Literal["all", "7d", "1m", "3m", "6m", "12m"] = "all"):
    #     await ctx.defer()
    #     await self.albums(ctx, period)
    #
    # @_last.command(name="tracks", description="Fetch your most played tracks")
    # @describe(period="Time period")
    # async def _tracks(self, ctx: Context, period: Literal["all", "7d", "1m", "3m", "6m", "12m"] = "all"):
    #     await ctx.defer()
    #     await self.tracks(ctx, period)
    #
    # @_last.command(name="my", description="Share your last.fm profile link")
    # async def _my(self, ctx: Context):
    #     await self.my(ctx)

    @command(name="lyricsgenius",
    ...
    async def _lyrics(self, ctx: Context, search_query=None):
        await ctx.defer()
